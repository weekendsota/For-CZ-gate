...
numlists=[]
with open(iDir+'/further_conditions2.csv') as f:
    for row in csv.reader(f):
        numlists.append(row[0])
indexlist=list(dict.fromkeys(numlists))
print(indexlist)

for index in indexlist:
    dirpath=iDir+'/Pics_instantlook/Cc='+str(index)
    if os.path.exists(dirpath)==False:
        os.mkdir(iDir+'/Pics_instantlook/Cc='+str(index))
...
for condition in tqdm(condition_lists):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(iDir+"/Pics_instantlook/Cc="+str(round(Cc,3))+"/Cc="+str(Cc)+",θf="+str(the_f)+"×π_2,J="+str(round(J0,3))+"[GHz_2π],tg="+str(tg)+"[ns].jpg")